...
from urllib.parse import parse_qs, parse_qsl, quote_plus
...
class BasePayment(object):
    ...
    def generate_check_value(self, params):
        ...
        if encrypt_type == 1:
            check_mac_value = (
                hashlib.sha256(encoding_str.encode("utf-8")).hexdigest().upper()
            )
        elif encrypt_type == 0:
            check_mac_value = (
                hashlib.md5(encoding_str.encode("utf-8")).hexdigest().upper()
            )
...
class CreateOrder(BasePayment):
    ...
    def create_order(self, client_parameters):
        ...
        default_parameters = self.create_default_dict(self.__ORDER_REQUIRED_PARAMETERS)
        ...
        if (
            ...
            or choose_payment == ChoosePayment["ATM"]
        ):
        ...
        if (
            choose_payment == ChoosePayment["ALL"]
            or choose_payment == ChoosePayment["CVS"]
            or choose_payment == ChoosePayment["BARCODE"]
        ):
            payment_extend_parameters = self.create_default_dict(
        ...
        if (
            choose_payment == ChoosePayment["ALL"]
            or choose_payment == ChoosePayment["Credit"]
        ):
            payment_extend_parameters = self.create_default_dict(
        ...
        if (
            client_parameters.get("ChooseSubPayment")
            == ChooseSubPayment["WebATM"]["TACHONG"]
            or client_parameters.get("ChooseSubPayment")
            == ChooseSubPayment["WebATM"]["SINOPAC"]
        ):
            raise Exception(
        ...
        if (
            ...
            # 使用 ALL 或 Credit 付款方式: 一次付清(三擇一)
            if client_parameters.get("Redeem") or client_parameters.get("UnionPay"):
            ...
            # 使用 ALL 或 Credit 付款方式: 定期定額(三擇一)
                ...
                client_parameters.get("PeriodAmount")
                or client_parameters.get("PeriodType")
                or client_parameters.get("Frequency")
                or client_parameters.get("ExecTimes")
                or client_parameters.get("PeriodReturnURL")
            ):
                credit_extend_parameters = self.create_default_dict(
        ...
        if client_parameters.get("InvoiceMark") == "Y":
            ...
            # 合併
            default_parameters = super().merge(default_parameters, invoice_parameters)
            ...
            if (print_param == "1") and client_parameters.get("CarruerType"):
                raise Exception('CarruerType do not fill any value, when Print is "1".')
            ...
            if (donation_param == "1") and (print_param == "1"):
                raise Exception('Print have to fill "0", when Donation is "1".')
            # 若捐贈註記 Donation = '1' (捐贈)時，LoveCode 須有值
            love_code = client_parameters.get("LoveCode")
            if (donation_param == "1") and (not love_code):
                raise Exception('LoveCode have to fill value, when Donation is "1".')
            if love_code:
                if len(love_code) < 3 or len(love_code) > 7:
                    raise Exception("LoveCode have to fill fixed length of 3~7 digits.")

            urlencode_parameters = [
                "CustomerName",
                "CustomerAddr",
                "CustomerEmail",
                "InvoiceItemName",
        ...
        self.final_merge_parameters = self.integrate_parameter(
            self.final_merge_parameters, self.__check_pattern
        )
...
class OrderSearch(BasePayment):
    ...
    def order_search(self, action_url=__url, client_parameters={}):
        ...
        # 先用 required.dict 設定預設值並產生新 new.required.dict
        default_parameters = dict()
        default_parameters = self.create_default_dict(self.__ORDER_SEARCH_PARAMETERS)
        ...
        # 回傳給 client
        response = super().send_post(action_url, self.final_merge_parameters)
...
class DownloadMerchantBalance(BasePayment):
    ...
    def download_merchant_balance(self, action_url=__url, client_parameters={}):
        ...
        self.__check_pattern.append(self.__DOWNLOAD_MERCHANT_BALANCE_PARAMETERS)
...
class SearchSingleTransaction(BasePayment):
    ...
    def search_single_transaction(self, action_url=__url, client_parameters={}):
        ...
        self.__check_pattern.append(self.__SEARCH_SINGLE_TRANSACTION_PARAMETERS)
...
class DownloadDisbursementBalance(BasePayment):
    ...
    def download_disbursement_balance(self, action_url=__url, client_parameters={}):
        ...
        self.__check_pattern.append(self.__DOWNLOAD_DISBURSEMENT_BALANCE_PARAMETERS)
...
a = [
    ...
    OrderSearchPeriodic,
    CreditDoAction,
    DownloadMerchantBalance,
    SearchSingleTransaction,
    DownloadDisbursementBalance,
    ExtendFunction,
]
